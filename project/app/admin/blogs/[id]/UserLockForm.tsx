...
import { lockedBlog } from '@/app/action/blog';
...
export default function UserLockForm({ user }: Props) {
  const router = useRouter();
  const [locked, setLocked] = useState(!!user.lockedAt);
  ...
  const handleToggle = async () => {
    ...
    setLocked(newLocked);
    await lockedBlog(user.id, { locked: newLocked });
    router.refresh();
    setLoading(false);
  }
