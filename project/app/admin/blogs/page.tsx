import { getAllBlog } from '@/app/action/blog';
import Link from 'next/link';

export const dynamic = 'force-dynamic';

export const metadata = {
  title: 'administration des blogs',
  description: 'Page admin des blogs de la plateforme',
}

export default async function BlogList({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const { page } = await searchParams;
  const currentPage = Number(page) || 1;
  const { blogs, totalPages } = await getAllBlog(currentPage);
          ...
          <tbody>
            {blogs.map((blog) => (
              <tr key={blog.id} className="border-b">
                ...
                <td className="py-2">{new Date(blog.createdAt).toLocaleDateString('fr-FR')}</td>
                <td className="py-2 flex gap-2">
                  <Link href={`/admin/blogs/${blog.id}`} className="text-blue-500 hover:underline">
                    Modifier
                  </Link>
                </td>
              </tr>
