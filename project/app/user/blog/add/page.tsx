...
import { createBlogPost } from '@/app/action/blogPost';
...
export default function BlogPostAdd() {
  const router = useRouter();
  const { register, handleSubmit, control, formState: { errors, isSubmitting } } = useForm<FormData>({
    defaultValues: { title: '', content: '', picture: '' }
  });

  const onSubmit = async (data: FormData) => {
    await createBlogPost({ title: data.title, content: data.content, picture: data.picture || undefined });
